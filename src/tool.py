...
from functions import (
    ...
    custom_frustum,
    gen_description_pt,
    gen_description_line,
    gen_description_conf_int,
    gen_description_buildings,
    calcul_incert_pla_factor,
    shp2kml,
    llh_2_XYZ,
    XYZ_2_ENh,
    llh_2_llH,
...
import pyproj                    # Navigation rinex management
import numpy as np                     # Coordinates transformations
import fiona                        # Shapefile management
import simplekml                    # KML management

   # Navigation rinex management
...
def csv_to_kml(input_file, 
               ...
               show_buildings=True,
               margin=20, # float
               buildings='',      # Chemin vers le fichier .shp des bâtiments
               departments='',
               ...
               fr_beta=0,
               fr_gamma=0,
               temporal=False,    # Mode temporel activé ou non
               start_time="8h00", 
               end_time="20h00", 
               date="01/01/2000",
               dist_step=5,
    ...
    # Si le mode temporel est activé, on exécute la branche correspondante
    if temporal:
        if input_type == "kmltraj" and rinex_name:
            if buildings != "":
                dummy_kml = simplekml.Kml()
                buildings_dict = shp2kml(buildings, dummy_kml)
                if not quiet:
                    print(f"{len(buildings_dict)} bâtiments chargés depuis {buildings}")
            else:
                print("Les buildings n'existent pas")
                buildings_dict = {}
            df_optimal = compute_optimal_window_from_kml(
                kml_file=input_file,
                rinex_nav_file=rinex_name,
            ...
            print(df_optimal)
        elif input_type in ["extevent", "log"]:
            ...
            try:
                ...
                print(f"Erreur lors de la lecture du fichier '{input_file}' avec le séparateur '{separator}': {e}")
                return
            try:
                data.columns = labels
            except Exception as e:
            ...
            if input_type == "log":
                if buildings != "":
                    ...
                    buildings_dict = {}
                df_optimal = compute_optimal_window_from_log(
                    data=data,
                    rinex_nav_file=rinex_name,
                    buildings_dict=buildings_dict,
                    time_step_sec=time_step_sec,
            ...
            print("Type d'entrée non supporté pour le mode temporel.")
        return  # On sort de la fonction après le traitement temporel
